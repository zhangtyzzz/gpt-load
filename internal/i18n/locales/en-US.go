...
var MessagesEnUS = map[string]string{
	...
	"config.failover_status_codes":           "Failover Status Codes (Legacy)",
	"config.failover_status_codes_desc":      "Legacy compatibility setting. New proxy error handling is controlled by Error Handling Policy; configure status-code behavior there.",
	...
	"config.key_affinity_default_ttl":        "Default Key Affinity Cache Time (seconds)",
	"config.key_affinity_default_ttl_desc":   "Default expiration time (seconds) when a key affinity rule does not set its own cache time. Values less than or equal to 0 fall back to 3600 seconds.",
