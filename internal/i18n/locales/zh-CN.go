...
var MessagesZhCN = map[string]string{
	...
	"config.failover_status_codes":           "故障转移状态码（兼容保留）",
	"config.failover_status_codes_desc":      "兼容旧版本的配置项。新的代理错误处理以“错误处理策略”为准，请优先在那里配置每个状态码的处理方式。",
	...
	"config.key_affinity_default_ttl":        "默认 Key 亲和缓存时间（秒）",
	"config.key_affinity_default_ttl_desc":   "Key 亲和性规则未单独设置缓存时间时使用的默认过期时间（秒）。小于等于 0 时按 3600 秒处理。",
