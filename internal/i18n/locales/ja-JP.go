...
var MessagesJaJP = map[string]string{
	...
	"config.failover_status_codes":           "フェイルオーバーステータスコード（互換用）",
	"config.failover_status_codes_desc":      "旧バージョン互換の設定です。新しいプロキシのエラー処理はエラー処理ポリシーで制御されるため、ステータスコードごとの動作はそちらで設定してください。",
	...
	"config.key_affinity_default_ttl":        "デフォルト Key アフィニティキャッシュ時間（秒）",
	"config.key_affinity_default_ttl_desc":   "Key アフィニティルールに個別のキャッシュ時間がない場合に使用するデフォルト有効期限（秒）。0 以下の場合は 3600 秒として扱います。",
